...
function install_es_proxy() {
  ...
  # install patch - not present in BeanStalk
  [ -f /usr/bin/patch ] || yum -y install patch || {
    echo "$0 - [ERROR] failed to install patch" >&2
    exit 1
  }
  # validate that the patch executable is installed
  [ -f /usr/bin/patch ] || {
    echo "$0 - [ERROR] patch executable file not found" >&2
    exit 1
  }

  # XXX patching in place as a temporary workaround for EMR large payloads
  # we should create a more permanent solution with an option
  patch -f `readlink -f /usr/bin/aws-es-kibana` <<'EOF'
--- a/index.js
+++ b/index.js
...
+app.use(bodyParser.raw({limit: '2mb', type: function() { return true; }}));
 app.use(getCredentials);
 app.use(function (req, res) {
     var bufferStream;
EOF
  [ $? ] || {
    echo "$0 - [ERROR] failed to patch limit in aws-es-kibana" >&2
    exit 1
  }
